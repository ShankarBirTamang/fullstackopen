...
describe("Blog", () => {
  ...
  test("calls onLike twice when like button is clicked twice", async () => {
    ...
    // Verify that onLike was called exactly twice
    expect(mockOnLike).toHaveBeenCalledTimes(2);

    // Verify that onLike was called with the blog object both times
    expect(mockOnLike).toHaveBeenNthCalledWith(1, mockBlog);
    expect(mockOnLike).toHaveBeenNthCalledWith(2, mockBlog);
  });
});
